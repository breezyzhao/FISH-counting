...
new_folder_stucture=False  #set to True if analyzing images directly from the microscope folders   ### For Somayeh's tif files

base_dir="/Users/zhaox12/Desktop/Teresas_lab/project/15.FISH_spot_finding/raw_data/Roni/D29 FISH Pictures" #parent folder of the data
work_dir="/Users/zhaox12/Desktop/Teresas_lab/project/15.FISH_spot_finding/test/May10.Roni/D29 FISH Pictures"
...
if(do_finding):
    ...
    for folder_i, folder in enumerate(folders):
        ...
        if(new_folder_stucture):
            file_list  = sorted(get_files_from_dirs(base_dir + '/' + folder))  # [ [g,r,b], [g,r,b], ...]
            if (len(file_list) == 0):
                print("No files found in folder:",folder)
                continue
        else:
            file_list = sorted(glob.glob(base_dir + '/' + folder + '/*_DAPI.tif'))
            if(len(file_list) == 0):
    ...
    for folder_i, folder in enumerate(folders):
        ...
        for type in spot_counts[folder].keys():
            ...
            if(len(spot_counts[folder][type])>0):
                ret=plt.hist(spot_counts[folder][type], np.arange(0, 10, 0.25), histtype='step')
